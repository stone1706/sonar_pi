...
def run_cmd(cmd, silent=False):
    ...
    if status:
        ...
        rsonare Exception(msg)

def assert_is_exe(path):
    if not path:
        rsonare Exception('path is empty.')
    if not os.path.isfile(path):
        rsonare Exception('%r is not a file.' %path)
    if not os.access(path, os.X_OK):
        rsonare Exception('%r is not executable by this user.' %path)
